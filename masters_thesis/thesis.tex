...
infrastructure.
% why is matters? why it is interesting?
Solution is implemented as a LLVM optimization pass and is integrated into the
...
execute above procedure many times, because it consists of many manual steps
which is time consuming to perform. Ideally, there could be script that takes
line of code as an input and produces output with the value of the selected
target
...
would accept line of code as an input
, run analysis on where the execution flow in the program would occur to
get to the target instruction and transplant subset of the program into
separate binary.
...
preferably using LLVM infrastructure. It should easily allow
user to provide input of choice.
...
Experiments and their results are discussed in the chapter
...
The LLVM Intermediate Representation of the foo.c is presented in the following
code:
...
LLVM programs are composed out of units called \textbf{modules}. Two or more
...
\autoref{fig:llvm_module} presents the simple architecture of the LLVM module.
...
A \textbf{basic block} is a type of container that contains instructions that
are execute sequentially.\cite{llvm-bb} Each basic blocks is formed with
...
entry as it processor and thus form branching.

\section{Optimizations}
\label{sec:llvm-opt}

LLVM uses the concept of passes for the optimizations. Concrete optimizations
are implemented as a \textbf{pass} that works with some portion of program code
(e.g.  Module, Function, Loop, etc.) to collect or transform this portion of
the code.
...
\begin{myEnumerate}
\item \textbf{Analysis passes:} Collect information from the IR and feed it into
...
\item \textbf{Transform passes:} Change the program in some way. They can use
some analysis pass that has been ran before and produced some information.
...
\item \textbf{Utility passes:} Do not fit into analysis passes or transform passes categories.
...
Clang uses LLVM infrastructure for optimizations and code generation (as is
described in \autoref{fig:llvm_arch}).
...
Clang also is able to emit IR for the add.c with the following simple command:
...
Upon finishing execution, apex.py will run the extracted program automatically.
Besides running extracted program, apex.py creates build directory with the
