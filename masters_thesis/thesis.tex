...
found weather they can be marked as dead and removed or not.
...
Unfortunately, the algorithm presented above works correctly only on the simple,
non-complex inputs that are similar to the \mintinline{text}{example.s}.
We will present two slightly more complicated programs to reveal the flaws of
this algorithm.
In order for method to work correctly, algorithm for eliminating dead components
has to be extended to account for the introduced input complexities.
