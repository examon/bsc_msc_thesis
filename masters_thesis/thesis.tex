...
In the case of \mintinline{text}{foo.s}, we can see that the function
\mintinline{text}{foo} contains four basic blocks: \mintinline{text}{entry},
\mintinline{text}{if.then}, \mintinline{text}{if.else} and
\mintinline{text}{return}.

Basic block \mintinline{text}{entry} is special in a way that it is the first
basic block in the \mintinline{text}{foo} function and thus is executed always
first.
It is also prohibited to have any predecessors, unlike basic block
\mintinline{text}{if.then}, which have \mintinline{text}{entry} as it predecessor
...
Getting the AST from the code \mintinline{text}{add.c} is as simple as running
...
Clang is also directly able to emit IR for the \mintinline{text}{add.c} with
...
(the \mintinline{text}{main} function).
...
The emitted LLVM IR is shown in \mintinline{text}{example.s}:\footnote{
...
% next point
The target line number 7 from the \mintinline{text}{example.c}
corresponds to the line 16 from the \mintinline{text}{example.s}.
...
\mintinline{text}{example.s} with respect to the source and the given target,
we get output as shown in \mintinline{text}{example_extracted.s}:
...
As we can see from the \mintinline{text}{example.c}, the execution path from
the program entry to the \textbf{target} does not include the function
\mintinline{text}{foo} and its associated instructions.
We are left only with the function \mintinline{text}{bar} which contains
target and the necessary instructions in the function \mintinline{text}{main}.

The source given in \mintinline{text}{example_extracted.s} can be recompiled
...
In order to compute the data dependency graph, we use the \mintinline{text}{dg} library.
\cite{dg}
\footnote{
...
To illustrate how the graph can be used, we take a closer look at the
instruction: \mintinline{llvm}{%some_int = alloca i32, align 4}.
...
their operand.
If we removed \mintinline{llvm}{%some_int = alloca i32, align 4} without taking
into consideration that there are two instructions that depend on it, we would
get into inconsistent state where two instructions would contain
undefined values as their operand.\footnote{ More about undefined values at
...
For the purposes of computation, we treat this subgraph as it would have
undirected edges.

We use the data dependency graph computed in \autoref{sec:design-dep}
and we find its connected components using the following algorithm:

\begin{minted}[label=finding components,escapeinside=||,frame=lines,breaklines,framesep=10pt]{text}
...
As an example, we present components for \mintinline{text}{main} function
...
The \mintinline{text}{main} function contains four distinct components.
We can see from the computed call graph, that \mintinline{text}{foo} is being
called from the \emph{yellow} component, and \mintinline{text}{bar} is being
...
\mintinline{text}{main} function and another from the function
\mintinline{text}{bar}.
...
function \mintinline{text}{bar} calls function \mintinline{text}{qux}, but
\mintinline{text}{qux} is not part of the computed path (only components with
the \emph{blue} color are part of the path, that is one component from
\mintinline{text}{main} and one from \mintinline{text}{foo}).

This is problematic for the basic algorithm that we introduced in the
\autoref{sec:design-removing}.
This basic algorithm would mark \mintinline{text}{qux} as dead and subsequently
remove this function.
However, this would break code integrity, because function
\mintinline{text}{qux} has to be called in order for the execution to correctly
proceed.
...
This corresponds to line 11 in the \mintinline{text}{example_mod1.c}.
...
\mintinline{text}{qux} and therefore, the function \mintinline{text}{qux} will
...
The function \mintinline{text}{qux} has not been removed which means that the
...
We now introduce branching into the main function of our examples shown in code
\mintinline{text}{example_mod2.c}.
The corresponding LLVM IR is shown in the \mintinline{text}{example_mod2.s} and
the branch instruction occurs at line 42.
Also, there appears two new basic blocks that this branch instruction
refers to:
...
0. Let CS = set of all components in the code
1. Let PATH = set of components on the path from source to target
|\textbf{2. FOR EACH component C in PATH:}|
      |\textbf{FOR EACH instruction I in C:}|
          |\textbf{IF instruction I is part of basic block handled by the branch instruction:}|
             |\textbf{FIND component with the branching instruction responsible for I and add it to the PATH}|
...
% intro
The method that we described in \autoref{chap:design} is implemented as a
LLVM pass called \linebreak \textbf{APEXPass}.
We have decided to implement the method as an pass because of the great
capabilities provided by the LLVM infrastructure.
Leveraging LLVM, APEXPass can be ran as an optimization step along with other
LLVM optimizations or executed separately with LLVM optimizer tool opt:

\begin{minted}[label=running APEXPass with opt,frame=lines,framesep=10pt,breaklines]{bash}
opt -o apex.bc -load libAPEXPass.so -apex -file=example.c -line=7 < example.bc 2> build/apex.log
\end{minted}

% apexpass input & output definition
Using \mintinline{text}{opt} command presented above, APEXPass is ran via
\mintinline{text}{apex} flag and defines two command line arguments:
\mintinline{text}{file} and \mintinline{text}{line}.
Argument \mintinline{text}{file} specifies the C source file where is the
target located (in our case \mintinline{text}{example.c}) and argument
\mintinline{text}{line} specifies number of the line in the file (in the
example, line number 7).

Input to the \mintinline{text}{opt} is the \mintinline{text}{example.bc}, which
is the bitcode representation of the \mintinline{text}{example.c} with included
source-level debug information.
The reason why we need debug information included in the
\mintinline{text}{example.bc} is explained in the
\autoref{sec:impl_inst_locate}.

We can get \mintinline{text}{example.bc} by compiling
\mintinline{text}{example.c} using clang compiler:

\begin{minted}[frame=lines,framesep=10pt]{bash}
clang -g -c -emit-llvm example.c -o example.bc
\end{minted}

Another necessary input to the \mintinline{text}{opt} is
\mintinline{text}{libAPEXPass.so}, which is the pass itself compiled into the
shared library.

APEXPass produces output \mintinline{text}{apex.bc}, which is the bitcode
representation of the extracted part from the \mintinline{text}{example.bc}.
This extracted program can be executed directly with the tool
\mintinline{text}{lli} as follows:
...
information about the ModulePass behavior\cite{llvm-module-pass}.

However, these disadvantages are not critical for our purposes and advantages
of having whole program abstracted as a single unit outweighs the disadvantages.

% === SUB-SECTION: APEXPass structure ==========================================
\subsection{Structure of the APEXPass}

In order to work correctly, each ModulePass has to override the
\mintinline{text}{runOnModule} method with the following signature:
...
to be part of the \mintinline{text}{runOnModule} method.
In this section, we are going to explain additional, implementation specific
steps that needed to be added to the method.
...
of the target in the \mintinline{text}{example.c}, but we do knot know exactly
what IR instructions are equivalent to the target.

Taking example from the \autoref{sec:method_example}, we have input program
\mintinline{text}{example.c} and target in the function bar at the line 7.

The target in the \mintinline{text}{example.c} is the following line of code:
\mintinline{C}{int y = 42;} which is represented by the following IR
instruction in the \mintinline{text}{example.s}:
...
The \mintinline{text}{example.s} with debug symbols is shown in the
\autoref{appendix:example_dbg}.
Once the \mintinline{text}{example.s} is compiled with debug information, we
can iterate from within the APEXPass over each instruction to find out its
parent file name and line number using the following code:\cite{llvm-dbg}

\begin{minted}[label=mapping C line of code to the IR instructions,frame=lines,framesep=10pt]{cpp}
...
Once the \mintinline{text}{Line} and \mintinline{text}{File} match with the
user input, we have found our target IR instruction.
...
7 from the \mintinline{text}{example.c}:
...
However, when we take as a target line number 2:
...
As we have mentioned in \autoref{sec:design-dep}, we have not implemented
...
some parts of it, especially \mintinline{text}{LLVMDependenceGraph}.
This structure provides data dependencies which APEXPass requires.
...
\begin{myItemize}
\item Load instruction \mintinline{text}{X} that takes value of the target and
stores it into the separate register.
\item Call instruction to extract function with \mintinline{text}{X} as an
function argument.
\item Call instruction to exit function which will terminate execution.
\end{myItemize}

Taking \mintinline{text}{example_extracted.s} from the
\autoref{sec:method_example} (with the target corresponding to the instruction
\mintinline{llvm}{store i32 42, i32* %y, align 4}) and injecting exit and
extract functions after the target produces the final result presented in the
\mintinline{text}{example_extracted_final.s} code:
...
Because these instructions call external functions, their definitions have to
be linked to the program. This is done by the \mintinline{text}{apex.py} and is
discussed in the \autoref{sec:impl_launcher}.
...
Extraction is limited by the type of extracted value.
We observe in the presented code \mintinline{text}{example_extracted_final.s},
that the type of \mintinline{llvm}{%y} is \mintinline{llvm}{i32} (integer).
...
is in the consistent state. APEXPass works with the code that contains debug
symbols.
Problem is that the three newly injected instructions that were presented in
the \autoref{sub:impl_inject} are not containing debug symbols.
Having parts of the code with debug symbols and other parts without creates
inconsistent state.
...
\begin{minted}[label=stripping IR module of debug information,frame=lines,framesep=10pt]{cpp}
...
freely accessible in the following repository:
\url{https://github.com/examon/APEX}.
APEX serves as an user-friendly wrapper around APEXPass and providing an easy
way to run APEXPass without leaving user to worry about the internal structure.

% === SUB-SECTION: apex.py =====================================================
\subsection{apex.py}

There are multiple steps that need to be executed in order to properly
run APEXPass. Therefore, we have implemented script \mintinline{text}{apex.py}
that simplifies this process to the absolute minimum.

Running APEXPass via launcher \mintinline{text}{apex.py} does require only
Python interpreter.
The following example demonstrates how to run APEXPass with the input
\mintinline{text}{example.bc}, with target located in the file
\mintinline{text}{example.c} on the line 2.

\begin{minted}[label=running APEXPass via apex.py,frame=lines,framesep=10pt]{bash}
python apex.py example.bc example.c 2 --export=true
\end{minted}

\mintinline{text}{apex.py} launcher has three mandatory command line arguments
and one optional.
First argument is input code in the LLVM bitcode format.
Second argument is the target file and third is target line of code in the file.
Optional argument is boolean flag and when set to true,
\mintinline{text}{apex.py} will export call graphs of the input and also the
extracted program.

Upon finishing execution, executable called \textbf{extracted} is created.
This executable contains extracted part of the program and can be run just like
any UNIX executable:
...
Besides producing expected executable, \mintinline{text}{apex.py} also creates
build directory with the following important files:

\begin{myItemize}
\item \mintinline{text}{apex.bc}: Extracted program in the LLVM bitcode format
that can be run separately via tool \mintinline{text}{lli}.
\item \mintinline{text}{apex.log}: Log produced by the APEXPass.
\end{myItemize}

% === SUB-SECTION: linking apexlib =============================================
\subsection{Linking apexlib and Input}

The important step in the APEX execution is linking apexlib with the user
...
Linking itself is performed in the following three steps:
\begin{myEnumerate}
\item Compile \mintinline{text}{apexlib.c} into the LLVM bitcode
\mintinline{text}{apexlib.bc}.

\mintinline{text}{clang -g -c -emit-llvm apexlib.c -o apexlib.bc}

\item Link \mintinline{text}{apexlib.bc} with the user input (e.g.
\mintinline{text}{example.bc}) and produce \mintinline{text}{linked.ll} IR
code.

\mintinline{text}{llvm-link apexlib.bc example.bc -S -o=linked.ll}

\item Run assembler on the \mintinline{text}{linked.ll} and produce bitcode \mintinline{text}{linked.bc}.

\mintinline{text}{llvm-as build/linked.ll -o build/linked.bc}

\end{myEnumerate}

These three steps will essentially take user input
\mintinline{text}{example.bc}, link to it library with the exit and extract
functions \mintinline{text}{apexlib.bc} and produce output
\mintinline{text}{linked.bc} that will now serve as an input to the APEXPass
instead of the original input \mintinline{text}{example.bc}.

\mintinline{text}{linked.ll} is available for comparison with
\mintinline{text}{example.s} in the \autoref{appendix:linked}.
...
The first experiment is conducted on the code
\mintinline{text}{example_mod2.c}, which we introduced in
\autoref{subsec:path_branching}.
We picked this simple code in order to demonstrate the whole process of testing
APEX capabilities.
...
First, we compile \mintinline{text}{example_mod2.bc} into the LLVM bitcode with
debug information:
...
We run \mintinline{text}{-dot-callgraph} pass via \mintinline{text}{opt} tool
and dot with the following commands to generate call graph image from the
\mintinline{text}{example_mod2.bc} (\autoref{fig:exp_img_mod2}):
...
requirements for APEX input and can therefore run \mintinline{text}{apex.py} to
start extraction.
...
\mintinline{text}{_apex_exit} is called. APEX also generated
\mintinline{text}{build/apex.bc} file, which is the extracted program in the
LLVM bitcode format.
We can generate the call graph using the same method as we did with the
\mintinline{text}{example_mod2.bc} and get \autoref{fig:exp_img_mod2_target16}.
...
Full C source code that implements yes is shown in the \mintinline{text}{yes.c}
listing:
...
After compiling \mintinline{text}{yes.c} into the bitcode
\mintinline{text}{yes.bc}, we pick \mintinline{C}{int needed_size} from line 17
as a target.
...
Finally, lets test the else branch of the \mintinline{text}{yes.c} by picking
line 34 as a target: (\mintinline{C}{output_len = 2;}):
...
with the complexity of the \mintinline{text}{yes.c} source code.
...
We developed a LLVM transform pass (APEXPass) that implements the presented
...
comparable in complexity to a small UNIX tools.
...
As we mentioned in \autoref{sub:impl_inject}, extraction is currently
limited by the type of the extracted value.
Possible improvement of the extraction procedure would be to implement
extraction functions for every IR type and thus extending
\mintinline{text}{apexlib.c}.
...
optimization criteria (e.g. shortest path, path with fewest external
...
the following repository: \url{https://github.com/examon/APEX}

Please refer to the \mintinline{text}{README.md} for instructions about building
and running APEX.

Examples used in this thesis are included in the directory
