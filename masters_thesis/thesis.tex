...
The first experiment will be conducted on the code example\_mod2.c, which we
introduced in \autoref{subsec:path_branching}.
...
We run -dot-callgraph pass via opt tool and dot with the following commands to generate call graph from the
example\_mod2.bc (\autoref{fig:exp_img_mod2}):
...
% target 16
\begin{minted}[frame=lines,framesep=10pt,breaklines]{shell}
