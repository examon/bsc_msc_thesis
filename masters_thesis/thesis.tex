...
Method for extraction of the subprogram from the original source code.

\item{what is the solution of the problem?}
What we do is based on the static analysis of the code and extracting
relevant part of the program to the separate binary.
Leveraging the LLVM infrastructure.

\item{why is matters? why it is interesting?}
Solution is implemented as a tool APEX. It can extract subprograms
into standalone binaries.
APEX can repeatedly run extracted part of the program without any other
intermediary manual steps.

\item{results}
We have tested APEX on the set of 10 programs.
...
itself compiled into the shared library. More information about the build
process is discussed in the \autoref{sec:impl_launcher}.
...
APEXPass can be defined within the LLVM framework as a \textbf{ModulPass}
...
To name a few, function bodies are are referred by no particular order sinde
ModulePass handles entire program as a single unit. Also, there are no possible
optimizations to be done for ModulePass because optimizer does not have enough
information about behaviour of the ModulePass.\cite{llvm-module-pass}
...
additional, implementation specific steps (in bold) that needed to be added to
the method.

We add three additional steps the original five method steps described in the
\autoref{sec:method_overview} (step 1, 7 and 8). We also explain implementation
specific details behind step 2 in the subsequent subsection.
...
\item Find path from source to target in the call graph.
\item Eliminate dead components and functions that do not depend on the path.
\item{\textbf{Inject exit and extract functions from apexlib into the code.}}
...
The example.s with debug symbols is shown in the \autoref{appendix:example}.
...
IR instruction. Note that there may be more IR instructions per one target
(it depends on how complex the target is in the example.c).
...
After having found the target instruction, we need to ensure that we get the
data out of it and present it to the user. In order to accomplish this, we
inject load instruction after the target. This way we load the value of the
target into separate register and we use this register as an argument for the
call to the dumping function that we inject right after.
The last instruction that we inject at the end is call to exit, to ensure that
the extracted program in apex.bc ends right after the target value is extracted.

We disassemble bitcode contents of the apex.bc into human readable IR
and save it into the apex.ll file using llvm-dis tool:
...
\begin{minted}[label=apex.ll,frame=lines,framesep=10pt,linenos]{llvm}
define i32 @bar() #0 {
entry:
  %y = alloca i32, align 4
  store i32 42, i32* %y, align 4
...
\subsection{compiling apexlib and linking input with apexlib and produce bitcode}

\subsection{running apexpass with opt}

\subsection{exporting call graph, dependency graph, disassembly bc}

\subsection{running final binaries, save result and logs}
