...
1. Run |\emph{Breadth-first search} \cite{clrs}| on G to visit each instruction
...
\begin{minted}[label=constructing call graph,escapeinside=||,frame=lines,framesep=10pt]{text}
...
program execution flow path between specific components within the program in
relation to their dependencies. We can find path from source to target and know
which components this path contains.
